...
The original end-bunchiness research note is not retained in the tracked repository.
See the [current alignment reference](../alignment/README.md) and
[retained sweep results](../alignment/sweep-results.md) for current contracts and evidence.
...
The original bulk migration script was a local experiment and is not included in
the tracked repository. Use the migration guidance here; do not assume the old script is available.

## Writing a new dungeon in 1.3.0

Use this skeleton (see the current [create-dungeon skill](../../.claude/skills/create-dungeon/SKILL.md)):
...
- [Config validator](../../lib/core/config-validator.js) — resolves both primitives, applies macro defaults
- [Macro presets](../../lib/templates/macro-presets.js) — macro preset definitions + `resolveMacro`
- [Soup presets](../../lib/templates/soup-presets.js) — soup preset definitions (no birth fields)
- [User loop](../../lib/orchestrators/user-loop.js) — activity allocation, pre-existing spread, future-event guard
- [Macro and rate tests](../../tests/unit/macro-and-rate.test.js) — usage examples + invariants
- [Current alignment docs](../alignment/README.md) — retained contracts and links to evidence; the original end-bunchiness note is not tracked
