...
- **Retention option keys are strict.** Unknown keys in a `retention`
  ...
  `carry_forward` survives as a deprecated alias for `carryForward`.
...
- `carry_forward` retention option → `carryForward`.
...
6. **Unchanged, no action**: `onProgress` payload contract (1.5.4 shape),
   `dungeonToJSON` / `extractComments`, `validateSchema` report shape,
