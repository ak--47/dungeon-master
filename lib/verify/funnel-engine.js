...
function runOneAttempt(sorted, startIdx, steps, exclusionSteps, options) {
	const { windowCheck, graceperiod, trackStepProperties, anchorOk } = options;
	...
	for (; i < sorted.length; i++) {
		...
		for (let s = reached + 1; s < numSteps; s++) {
			if (!eventMatchesStep(ev, steps[s])) continue;
			// v1.6.0 (P1.6.5): trend-interval anchor — an event outside
			// [fromMs, toMs) can never record step 0 (funnel_query.cpp:1398-1401
			// "step 0 in [start, stop)"); it stays eligible for later steps
			// with the same name.
			if (s === 0 && anchorOk && !anchorOk(t)) continue;
			matchedStep = s; break;
		}
		if (hasExclusions) {
...
export function evaluateFunnel(events, steps, options = {}) {
	...
	const opts = { windowCheck, graceperiod, trackStepProperties, anchorOk };
