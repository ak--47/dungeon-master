 ...
 * @property {number} convertedEnders emulator bookkeeping (not an ARB counter):
 *   converted flows ending here, derived from the same per-flow converted bit
 *   (flows.cpp:841-847) — powers the list output's per-path converted count,
 *   which ARB's node counters alone cannot express after uncommon-merging.
 ...
 *   where each path is `{ steps, count, converted }` (count = flows ending
 *   exactly there; converted = how many of those converted). sankey →
 ...
 */
export function aggregateFlows(flows, { output = 'list', cardinalityThreshold } = {}) {
		...
		for (let i = 0; i < f.steps.length; i++) {
			...
			if (!node) {
				node = {
					...
					convertedEnders: 0, children: new Map(),
				};
			...
			if (i === f.steps.length - 1) {
				node.dropoff += 1;
				if (f.converted) node.convertedEnders += 1;
			}
			cur = node;
		}
	}
	...
	const mergeNode = (dst, src) => {
		...
		dst.convertedEnders += src.convertedEnders;
	...
	if (output === 'list') {
		...
		(function walk(children, prefix) {
			for (const node of children.values()) {
				const steps = [...prefix, stepOut(node)];
				if (node.dropoff > 0) {
					paths.push({ steps, count: node.dropoff, converted: node.convertedEnders });
				}
				walk(node.children, steps);
			}
		})(trimmed, []);
		paths.sort((a, b) => b.count - a.count
			|| cmpStr(a.steps.map(s => s.label).join(KEY_SEP), b.steps.map(s => s.label).join(KEY_SEP)));
		return { paths, totalEntered, overallConversionRate };
	}
