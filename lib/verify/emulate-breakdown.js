...
export function emulateBreakdown(events, config) {
	...
	if (config.timeBucket) {
		const buckets = partitionByTimeBucket(events, config.timeBucket);
		const inner = { ...config, timeBucket: undefined };
		const out = [];
...
//
// Birth retention: for each user, find the first occurrence of `cohortEvent`
// → birth timestamp. For each day bucket N, check if the user has
// `returnEvent` on the UTC day (birth_day + N).
//
...
// value (Mixpanel's segment_event=FIRST mode). Each segment gets its own row
// per day bucket.
//
// NOT implemented: COMPOUNDED retention, CARRY_BACK, CONSECUTIVE_FORWARD,
// CALENDAR_START alignment, segment_event=SECOND. Documented in HOOKS.md.

const DAY_MS_RET = 86400 * 1000;

function utcDay(ms) {
	return Math.floor(ms / DAY_MS_RET);
}

function retention(events, { cohortEvent, returnEvent, dayBuckets = [1, 7, 14, 30], segmentBy, carry_forward = false, identityMap }) {
	...
	const cohorts = new Map(); // segmentValue → { users: Set, retained: Map<uid, Set<bucket>> }
	const ensureSegment = (seg) => {
		if (!cohorts.has(seg)) cohorts.set(seg, { users: new Set(), retained: new Map(), birthDayByUser: new Map(), returnDaysByUser: new Map() });
	...
	for (const [uid, evs] of userEvents) {
		...
		const birthDay = utcDay(birthMs);
		const seg = segmentBy ? (birth[segmentBy] ?? 'unknown') : 'all';
		const sb = ensureSegment(seg);
		sb.users.add(uid);
		sb.birthDayByUser.set(uid, birthDay);

		const retDays = new Set();
		for (const ev of sorted) {
			if (ev.event !== returnEvent) continue;
			const d = utcDay(toMs(ev.time));
			if (d > birthDay) retDays.add(d - birthDay); // day offset (>=1)
		}
		sb.returnDaysByUser.set(uid, retDays);
	}
	...
	for (const [seg, sb] of cohorts) {
		...
		for (const day of dayBuckets) {
			...
			for (const uid of sb.users) {
				const days = sb.returnDaysByUser.get(uid);
				if (!days) continue;
				if (carry_forward) {
					// Retained on day N if retained on any day in [1, N].
					let hit = false;
					for (const d of days) {
						if (d <= day) { hit = true; break; }
					}
					if (hit) retained++;
				} else {
