 ...
 *   anchor bounds (P1.6.5) — honored by the sequential orders only; the
 *   any-order completion helpers have no anchor concept.
 */
 ...
 * `funnelFrequency` and `timeToConvert` (sequential orders) and `retention`
 * follow Mixpanel's interval rule "step 0 in [start, stop); steps 1+ in
 ...
 * Non-sequential funnel orders keep plain event partitioning.
...
export function emulateBreakdown(events, config) {
	...
	if (config.timeBucket) {
		...
		//   - funnelFrequency / timeToConvert (sequential orders): slice to
		//     [start, stop + window) and pass `anchorRange` down so step 0
		//     only anchors in [start, stop).
		// Non-sequential funnel orders keep plain partitioning: the any-order
		// completion helpers have no anchor concept (revisit with P1.6.6).
		const seqOrder = !config.funnelOrder
			|| config.funnelOrder === 'sequential' || config.funnelOrder === 'interrupt' || config.funnelOrder === 'interrupted';
		const anchored = config.type === 'retention'
			|| ((config.type === 'funnelFrequency' || config.type === 'timeToConvert') && seqOrder);
				...
				if (config.type === 'retention') {
					...
					const spillMs = typeof config.conversionWindowMs === 'number'
						...
						: (config.conversionWindow && typeof config.conversionWindow.n === 'number'
							? config.conversionWindow.n * 86400_000 // session windows: n × 1-day wall-clock outer bound
							: Infinity);
...
function timeToConvert(events, { fromEvent, toEvent, breakdownByUserProperty, profiles = [], funnelOrder = 'sequential', conversionWindowMs, identityMap, reentry, exclusionSteps, sessionScoped, anchorRange }) {
	...
	for (const [uid, evs] of userEvents) {
		...
		const r = evaluateFunnelByOrder(evs, [fromEvent, toEvent], { funnelOrder, conversionWindowMs, reentry, exclusionSteps, sessionScoped, anchorRange });
