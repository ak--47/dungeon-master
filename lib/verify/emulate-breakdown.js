 ...
 *   event with the last-N touchpoint cap applied per conversion (Mixpanel behavior —
 *   attributed_value_reader_read takes one event_time_ms per conversion).
 *
...
//
// Touchpoint cap: max 10 touchpoints in lookback window (`TOUCHPOINTS_LIMIT
// = 10`, backend/libquery/properties_over_time/attributed_value_reader.cpp:16).
// For first/last touch the cap matters when the user has > 10 touches before
// conversion; the cap shifts which touches enter the candidate pool.
//
...
// builds a fresh capped lookback read ending at that conversion (same file,
// attributed_value_reader_create_params passes TOUCHPOINTS_LIMIT per read).
...
// the last-N touchpoints BEFORE each conversion. A user with more eligible
// events than the generator cap can have touches the lookback never sees —
// and stamped touches can postdate every conversion. In practice divergence
// is theoretical below ~10 eligible events per user (cap = window size);
...
function attributedBy(events, {
	...
	for (const [, evs] of userEvents) {
		...
		for (const conversion of conversions) {
			...
			// re-applied per conversion. (When touch count <= cap, a no-op.)
