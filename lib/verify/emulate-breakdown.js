...
function funnelFrequency(events, { steps, breakdownByFrequencyOf, conversionWindowMs, conversionWindow, periodUnit = 'day', funnelOrder = 'sequential', identityMap, reentry, exclusionSteps, trackStepProperties, sessionScoped, countMode, holdPropertyConstant }) {
	...
	const funnelOpts = { conversionWindowMs, conversionWindow, countMode, reentry, exclusionSteps, trackStepProperties, sessionScoped };
	for (const [, evs] of userEvents) {
		const b = countDistinctPeriods(evs, breakdownByFrequencyOf, /** @type {*} */ (periodUnit));
		...
		if (holdPropertyConstant) {
			...
			if (isTotalsMode) {
				...
				for (const r of perValue.values()) {
					for (const a of /** @type {Array<*>} */ (r)) reachedList.push(a.reached);
				}
			} else {
				...
				let best = -1;
				for (const r of perValue.values()) {
					best = Math.max(best, /** @type {*} */ (r).reached);
				}
				if (best >= 0) reachedList.push(best);
			}
		} else if (isTotalsMode) {
			const attempts = /** @type {Array<*>} */ (evaluateFunnel(evs, steps, funnelOpts));
			for (const a of attempts) reachedList.push(a.reached);
		} else {
			// v1.5: dispatch on funnel.order so non-sequential modes don't return 0% trivially.
			const r = evaluateFunnelByOrder(evs, steps, { conversionWindowMs, conversionWindow, funnelOrder, reentry, exclusionSteps, trackStepProperties, sessionScoped });
