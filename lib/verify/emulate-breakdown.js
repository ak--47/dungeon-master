...
function evaluateFunnelByOrder(userEvents, steps, options = {}) {
	...
	switch (order) {
		...
		case 'interrupt':
		case 'interrupted': {
			const r = evaluateFunnel(userEvents, steps, { conversionWindowMs: options.conversionWindowMs });
			return { ...r, mode: order, verificationKind: undefined };
		}
		case 'first-fixed': {
			const stepZero = evaluateFunnel(userEvents, [steps[0]], { conversionWindowMs: options.conversionWindowMs });
...
function funnelFrequency(events, { steps, breakdownByFrequencyOf, conversionWindowMs, periodUnit = 'day', funnelOrder = 'sequential' }) {
	...
	for (const [, evs] of userEvents) {
		...
		const r = evaluateFunnelByOrder(evs, steps, { conversionWindowMs, funnelOrder });
		const b = countDistinctPeriods(evs, breakdownByFrequencyOf, /** @type {*} */ (periodUnit));
...
function timeToConvert(events, { fromEvent, toEvent, breakdownByUserProperty, profiles = [], funnelOrder = 'sequential', conversionWindowMs }) {
	...
	for (const [uid, evs] of userEvents) {
		...
		const r = evaluateFunnelByOrder(evs, [fromEvent, toEvent], { funnelOrder, conversionWindowMs });
