 ...
 * @property {'frequencyByFrequency'|'funnelFrequency'|'aggregatePerUser'|'timeToConvert'|'attributedBy'|'sessionMetrics'|'retention'|'distinctCount'|'eventBreakdown'|'uniques'|'lifecycle'} type
 *
 ...
 * @property {'sessions'} [countType] - eventBreakdown + uniques (P1.7.3):
 *   count once per (user, session, segment) / distinct (user, session)
 *   pairs per bucket, deriving sessions via sessionize().
