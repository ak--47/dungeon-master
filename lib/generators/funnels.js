	...
	if (Array.isArray(funnel.exclusionEvents) && funnel.exclusionEvents.length
		...
		if (Number.isFinite(lastTimeMs)) {
			...
			for (let i = 0; i < numToInject; i++) {
				...
				const cloned = { ...lastEvent };
				// Override identity-irrelevant fields; keep user_id, device_id, distinct_id, etc.
				cloned.event = excName;
				cloned.time = new Date(lastTimeMs + offsetMs).toISOString();
