 ...
 * @param {Object} [persona] - User's assigned persona (Phase 2)
 * @returns {Promise<[Array, boolean]>} Tuple of [events, didConvert]
 */
export async function makeFunnel(context, funnel, user, firstEventTime, profile = {}, scd = {}, persona = null, phase2Ctx = {}) {
	...
	const resolvedWorldEvents = config.worldEvents;
	...
	if (resolvedFeatures && firstEventTime) {
		const daysSinceBegin = (firstEventTime - global.FIXED_BEGIN) / 86400;
	...
	// Build complete phase2 context: merge passed-in phase2Ctx with config fallbacks
	const funnelPhase2 = {
		persona: phase2Ctx.persona || persona || null,
		userCampaign: phase2Ctx.userCampaign || null,
		userLocation: phase2Ctx.userLocation || null,
		worldEventsTimeline: phase2Ctx.worldEventsTimeline || context.config.worldEvents || null,
		resolvedFeatures: phase2Ctx.resolvedFeatures || context.config.features || null,
		resolvedAnomalies: phase2Ctx.resolvedAnomalies || context.config.anomalies || null,
		dataQuality: phase2Ctx.dataQuality || context.config.dataQuality || null,
		geo: phase2Ctx.geo || context.config.geo || null,
	};
