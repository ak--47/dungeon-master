		...
		tables: specs.map((spec, index) => {
			...
			return {
				...
				columns: orderedColumns.map((columnName) => ({
					name: columnName,
					bqType: inferBqType(columnName, firstRow[columnName]),
				})),
...
function computeWindowValues({ spec, accumulator, seriesKey, windowBuckets }) {
	if (spec.type === 'point-in-time') {
		...
		for (const bucketSec of windowBuckets) {
			...
			values.push(roundTo(level * spec.scale, 2));
		}
		return values;
	}

	return windowBuckets.map((bucketSec) => roundTo(getBucketMeasureValue(spec, accumulator.getCell(spec.name, seriesKey, bucketSec), bucketSec) * spec.scale, 2));
}
...
function computeBackfillValues(spec, windowValues) {
	...
	for (let step = spec.history; step >= 1; step -= 1) {
		values.push(roundTo(Math.max(0, firstValue - (slope * step)), 2));
	}
	return values;
}
...
function buildRow({ spec, bucketSec, seriesKey, value, bucketIndex, bucketCount, grain, isBackfill, config }) {
	const row = {
		[spec.timeColumn]: new Date(bucketSec * 1000).toISOString().slice(0, 10),
	};
	const seriesValues = splitSeriesKey(spec, seriesKey);
...
function splitSeriesKey(spec, seriesKey) {
	if (!spec.source.groupBy || spec.source.groupBy.length === 0) return [];
	return String(seriesKey ?? '').split('|');
}
...
function inferBqType(columnName, value) {
	if (columnName) {
		if (typeof columnName === 'string' && /date|month/i.test(columnName) && typeof value === 'string') return 'DATE';
	}
	if (typeof value === 'number') return 'FLOAT64';
	if (typeof value === 'boolean') return 'BOOL';
	return columnName && /date|month/i.test(columnName) ? 'DATE' : 'STRING';
}
