...
export async function userLoop(context) {
	...
	for (let i = 0; i < numUsers; i++) {
		const userPromise = USER_CONN(async () => {
			...
			const dayPlan = (avgActiveDaysPerUser !== undefined && avgActiveDaysPerUser !== null && Number.isFinite(avgActiveDaysPerUser))
				? buildActiveDayPlan({
			...
			while (numEventsPreformed < numEventsThisUserWillPreform && !cancelled) {
				...
				const eventFeatureCtx = dayBounds
				...
				if (hasUsageFunnels && userConverted) {
					...
					const funnelCursor = dayBounds ? dayBounds.earliest : usageFunnelCursor;
					const [data, converted] = await makeFunnel(context, currentFunnel, user, funnelCursor, profile, userSCD, persona, eventFeatureCtx, usageAttemptMeta);
					numEventsPreformed += data.length;
					newEvents = data;
					if (data.length) {
						const lastTime = dayjs(data[data.length - 1].time).unix();
					...
					const data = await makeEvent(context, distinct_id, standaloneEarliest, u.pick(weightedEvents), user.anonymousIds, {}, config.groupKeys, false, false, eventFeatureCtx, standaloneIdentityCtx);
					numEventsPreformed++;
					newEvents = [data];
				}
 ...
 *
 * Returns an array of UTC day-start unix-seconds, length === numEvents. Each
 * element names the day on which the event-i should land. Events naturally
 * concentrate onto `targetActiveDays` distinct days (drawn from a normal around
 * `avgActiveDaysPerUser`, clamped to `[1, userActiveDays]`).
 *
 ...
 * @returns {number[] | null} Array of UTC day-start unix-seconds, or null if not
 *   applicable (no candidate days, no events, etc.).
 */
