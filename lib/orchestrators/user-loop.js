		...
		const userPromise = USER_CONN(async () => {
			...
			while (numEventsPreformed < numEventsThisUserWillPreform && !cancelled && iterationCount < MAX_ITERATIONS) {
				...
				if (hasUsageFunnels && userConverted) {
					...
					// Constrain funnel step1's TimeSoup latestTime to `FIXED_NOW - ttc` so
					// the full funnel fits in window. Without this, late steps spill past
					// FIXED_NOW and get `_drop`'d, causing the budget loop to over-iterate
					// and total events to stay roughly constant regardless of the world-event
					// `volumeMultiplier` (which uses the same `_drop` flag for semantic
					// suppression). Constraining step1 keeps spillover near zero so the
					// `_drop` flag retains its original semantic meaning ("suppressed").
					//
					// Born-late edge case: when `funnelCursor > FN - ttc`, fall back to
					// `[funnelCursor, FN]` so the user can still emit. Some late steps
					// will _drop, but the cluster anchors near birth, not at the FN boundary.
					const safeLatest = context.FIXED_NOW - ttcSec;
					...
					// Budget counts raw output (matches pre-fix semantics). With the
					// `funnelLatestTime` constraint above, very few events get `_drop`'d,
					// so the budget loop terminates at the expected event count and
					// world-event `_drop`s correctly reduce the user's surviving total.
					numEventsPreformed += data.length;
					newEvents = data;
				} else {
