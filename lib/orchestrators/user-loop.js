		...
		const userPromise = USER_CONN(async () => {
			...
			if (firstFunnels.length && userIsBornInDataset) {
				...
				for (let attemptNum = 1; attemptNum <= totalAttempts; attemptNum++) {
					...
					if (data && data.length) {
						if (failedPriors > 0) promisedAttemptEntries.add(data[0]);
					...
					if (failedPriors > 0 && (!data.length || data.some(event => event._drop))) {
						throw new RangeError(`Lifecycle capacity: cannot emit promised first-funnel attempt ${attemptNum}/${totalAttempts}`);
					}
				}
			...
			if (strictEventCount && usersEvents.length > 0) {
				const promisedEntries = usersEvents.filter(event => promisedAttemptEntries.has(event));
				...
				if (promisedEntries.some(event => !usersEvents.includes(event))) {
					throw new RangeError('Lifecycle capacity: strict event count removed a promised first-funnel attempt');
				}
			}
			...
			if (userIsBornInDataset && userDevicePool?.length) {
				...
				for (const event of usersEvents) {
					if (emittedAuthTime === null || Date.parse(event.time) < emittedAuthTime) {
						...
						event.device_id ||= userDevicePool[0];
					}
				}
				if (emittedAuthTime === null) profile._drop = true;
			}
