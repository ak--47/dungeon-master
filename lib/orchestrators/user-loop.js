	...
	for (let i = 0; i < numUsers; i++) {
		const userPromise = USER_CONN(async () => {
			...
			if (attribution && userIsBornInDataset) {
				...
				const birthDay = Math.max(0, (birthUnix - global.FIXED_BEGIN) / 86400);
			...
			// Build phase2 context for event generation
			const phase2Ctx = {
				persona,
				userLocation,
			...
			if (firstFunnels.length && userIsBornInDataset) {
				...
				const [data, converted] = await makeFunnel(context, firstFunnel, user, firstTime, profile, userSCD, persona, phase2Ctx);
			...
			while (numEventsPreformed < numEventsThisUserWillPreform && !cancelled) {
				...
				if (usageFunnels.length && userConverted) {
					...
					const [data, converted] = await makeFunnel(context, currentFunnel, user, userFirstEventTime, profile, userSCD, persona, phase2Ctx);
					numEventsPreformed += data.length;
					newEvents = data;
				} else {
					const data = await makeEvent(context, distinct_id, userFirstEventTime, u.pick(weightedEvents), user.anonymousIds, user.sessionIds, {}, config.groupKeys, true, false, phase2Ctx);
					numEventsPreformed++;
					newEvents = [data];
				}
...
async function generateBotUsers(context, dataQuality, storage) {
	...
	for (let b = 0; b < botUsers; b++) {
		...
		let currentTime = global.FIXED_BEGIN + context.TIME_SHIFT_SECONDS;
