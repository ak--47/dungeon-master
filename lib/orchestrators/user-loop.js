...
export async function userLoop(context) {
	...
	const {
		...
		dataQuality,
		// Killed in 1.4 — config-validator forces these to null. Local bindings retained
		// only so the existing `if (subscription)` etc. guards below no-op cleanly.
		subscription,
		attribution,
		geo,
		features,
		...
		const userPromise = USER_CONN(async () => {
			...
			while (numEventsPreformed < numEventsThisUserWillPreform && !cancelled) {
				...
				if (usageFunnels.length && userConverted) {
					...
					const data = await makeEvent(context, distinct_id, userFirstEventTime, u.pick(weightedEvents), user.anonymousIds, user.sessionIds, {}, config.groupKeys, true, false, featureCtx, standaloneIdentityCtx);
					numEventsPreformed++;
					newEvents = [data];
				}
			...
			if (hasSessionIds && usersEvents.length > 0) {
				...
				if (userDevicePool && userDevicePool.length > 1) {
					...
					for (const ev of usersEvents) {
						...
						if (!dev) {
							dev = chance.pickone(userDevicePool);
