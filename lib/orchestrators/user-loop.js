		...
		const userPromise = USER_CONN(async () => {
			...
			const dayPlanResult = (hasActiveDaysKnob || hasRetentionCurve)
				? buildActiveDayPlan({
					...
					hourWeights: soupCfgForActiveDay.hourOfDayWeights,
					retentionCurve,
					chance,
				})
				: null;
			...
			while (numEventsPreformed < numEventsThisUserWillPreform && !cancelled && iterationCount < MAX_ITERATIONS) {
				...
				if (dayBounds && !hasAvailableHours(usageEarliest, dayBounds.latest, soupCfgForActiveDay.hourOfDayWeights)) continue;
 ...
 * is proportional to those same weights. Partial-day slots are thinned once by
 * available duration and reassigned to full days when capacity exists. A picked
 * partial day can be empty; reassignment can add a full day to preserve budget.
 *
 ...
 * @param {number[]} [args.hourWeights] - 24-element soup hour weights (UTC)
 ...
 */
function buildActiveDayPlan({ adjustedCreated, fixedBegin, fixedNow, avgActiveDaysPerUser, userActiveDays, numEvents, dowWeights, hourWeights, retentionCurve, chance }) {
	...
	const lastDay = Math.ceil(userEndUnix / dayMs) - 1;
	const candidateDays = [];
	for (let d = firstDay; d <= lastDay; d++) {
		const dayStart = d * dayMs;
		if (hasAvailableHours(Math.max(dayStart, userStartUnix), Math.min(dayStart + dayMs - 1, userEndUnix), hourWeights)) {
			candidateDays.push(dayStart);
		}
	}
	...
	const availableFraction = dayStart =>
		(Math.min(dayStart + dayMs, userEndUnix) - Math.max(dayStart, userStartUnix)) / dayMs;
	...
	const fullDays = candidateDays.filter(dayStart => availableFraction(dayStart) >= 1);
	if (fullDays.length > 0) {
		let unallocated = 0;
		for (let index = 0; index < pickedDays.length; index++) {
			const fraction = availableFraction(pickedDays[index]);
			if (fraction >= 1) continue;
			const originalCount = allocations[index];
			let retainedCount = 0;
			for (let slot = 0; slot < originalCount; slot++) {
				if (chance.random() < fraction) retainedCount++;
			}
			allocations[index] = retainedCount;
			unallocated += originalCount - retainedCount;
		}
		if (unallocated > 0) {
			let recipients = pickedDays.map((dayStart, index) => ({ dayStart, index }))
				.filter(({ dayStart }) => availableFraction(dayStart) >= 1);
			if (!recipients.length) {
				const fullWeights = fullDays.map(dayStart => weights[candidateDays.indexOf(dayStart)]);
				const [dayStart] = weightedSampleNoReplacement(fullDays, fullWeights, 1, chance);
				const index = pickedDays.length;
				pickedDays.push(dayStart);
				allocations.push(0);
				pickedWeights.push(1);
				recipients = [{ dayStart, index }];
			}
			const recipientWeights = recipients.map(({ index }) => pickedWeights[index]);
			for (let slot = 0; slot < unallocated; slot++) {
				const recipient = chance.weighted(recipients, recipientWeights);
				allocations[recipient.index]++;
			}
		}
	}
	...
	const pickedDayBuckets = new Set(pickedDays.filter((_, index) => allocations[index] > 0).map(daySec => Math.floor(daySec / 86400)));
	// chance.shuffle uses seeded RNG → deterministic.
	return { plan: chance.shuffle(plan), pickedDayBuckets };
}

function hasAvailableHours(earliest, latest, hourWeights) {
	if (!Array.isArray(hourWeights) || hourWeights.length !== 24) return true;
	if (earliest === latest) return hourWeights[new Date(earliest * 1000).getUTCHours()] > 0;
	for (let hourStart = Math.floor(earliest / 3600) * 3600; hourStart < latest; hourStart += 3600) {
		if (hourWeights[new Date(hourStart * 1000).getUTCHours()] > 0) return true;
	}
	return false;
}
