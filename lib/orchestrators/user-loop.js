		...
		const userPromise = USER_CONN(async () => {
			...
			while (numEventsPreformed < numEventsThisUserWillPreform && !cancelled && iterationCount < MAX_ITERATIONS) {
				...
				if (hasUsageFunnels && userConverted) {
					...
					//
					// Round-3 fix (2026-05-09): cap dead zone at `FUNNEL_DEAD_ZONE_CAP_SEC`
					// (1 day) regardless of declared `timeToConvert`. Was naively
					// `latestTime = FN - ttc`, which created a 14-day right-edge dead zone
					// for any 14d-ttc funnel — making fitness (multiple long-ttc funnels)
					// measure tail_ratio = 0.27 on `macro: "flat"` (over-decline). With the
					// 1d cap, long-ttc funnels' step1 anchors right up to `FN - 1d`. Late
					// steps spill past FN and get `_drop`'d (legacy behavior), but the
					// per-cluster shape stops dragging the right edge down by `ttc` days.
					//
					// Trade-off: long-ttc funnels lose some events to `_drop` (legacy
					// behavior), but their per-cluster trapezoidal shape no longer dominates
					// aggregate distribution. Catch-all (`ttc=1d`, set in config-validator)
					// is unchanged.
					//
					// Born-late edge case: when `funnelCursor > FN - dead_zone`, fall back
					// to `[funnelCursor, FN]` so the user can still emit.
					const FUNNEL_DEAD_ZONE_CAP_SEC = 0; // Phase-3 Pass-1 trial: remove dead zone
