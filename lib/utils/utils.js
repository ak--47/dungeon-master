...
function TimeSoup(earliestTime, latestTime, peaks = 5, deviation = 2, mean = 0, dayOfWeekWeights = DEFAULT_DOW_WEIGHTS, hourOfDayWeights = DEFAULT_HOD_WEIGHTS) {
	...
	const peakCount = Math.ceil(peaks);
	const chunkSize = totalRange / peakCount;

	// Phase 1: Gaussian chunk sampling (macro trend across the time range)
	const sampleChunk = () => {
		const peakIndex = integer(0, peakCount - 1);
		const chunkStart = earliestTime + peakIndex * chunkSize;
		const offset = chance.normal({ mean, dev: chunkSize / deviation });
		const period = 2 * chunkSize;
		const folded = ((chunkSize / 2 + offset) % period + period) % period;
		return chunkStart + (folded <= chunkSize ? folded : period - folded);
	};
	let candidate = sampleChunk();
	...
	if (hourOfDayWeights) {
		const dayStart = Math.floor(candidate / 86400) * 86400;
		const availableStart = Math.max(dayStart, earliestTime);
		const availableEnd = Math.min(dayStart + 86400, latestTime);
		const weightedHours = (from, to) => {
			const intervals = [];
			for (let hourStart = Math.floor(from / 3600) * 3600; hourStart < to; hourStart += 3600) {
				const weight = hourOfDayWeights[new Date(hourStart * 1000).getUTCHours()];
				if (!(weight > 0)) continue;
				const start = Math.max(hourStart, from);
				const end = Math.min(hourStart + 3600, to);
				intervals.push({ start, end, weight: (end - start) * weight });
			}
			return intervals;
		};
		let intervals = weightedHours(availableStart, availableEnd);
		let totalWeight = intervals.reduce((total, interval) => total + interval.weight, 0);
		if (!(totalWeight > 0)) {
			intervals = weightedHours(earliestTime, latestTime);
			totalWeight = intervals.reduce((total, interval) => total + interval.weight, 0);
		}
		if (!(totalWeight > 0)) throw new RangeError('TimeSoup window has no positive weighted hours.');
		let roll = chance.random() * totalWeight;
		for (const interval of intervals) {
			if (interval.weight <= 0) continue;
			if (roll < interval.weight) {
				candidate = interval.start + (roll / interval.weight) * (interval.end - interval.start);
				break;
			}
			roll -= interval.weight;
		}
	}
