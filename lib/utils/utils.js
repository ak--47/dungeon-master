...
//
// `IdempotencyStrategy.RetryAlways` overrides the default "RetryConditional"
// (which only retries when SDK is sure the operation hasn't applied) so that
// resumable-upload PUTs retry even when their state is ambiguous — safe for
// our use case because we write append-only / overwrite-only files and a
// duplicate PUT to the same resumable session is a no-op.
...
function streamJSON(filePath, data, options = {}) {
	return new Promise((resolve, reject) => {
		...
		if (filePath?.startsWith('gs://')) {
			...
			const gcsStream = storage.bucket(bucket).file(file).createWriteStream({ resumable: false });
