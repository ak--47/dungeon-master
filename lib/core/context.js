...
export function createContext(config, storage = null, timeConstants = {}) {
	...
	const context = {
		...
		// Post-shift dataset boundaries — what hooks should anchor relative-time math to.
		// FIXED_BEGIN/FIXED_NOW are pre-shift (around 2024-02-02). The orchestrator
		// shifts every event timestamp by TIME_SHIFT_SECONDS so output ISO strings
		// land in the present window. Hooks see those post-shift timestamps, so
		// "day N of dataset" must be computed against these post-shift anchors.
		DATASET_START_SECONDS: (() => {
			const fixedBegin = timeConstants.fixedBegin || global.FIXED_BEGIN;
			const fixedNow = timeConstants.fixedNow || global.FIXED_NOW;
			const actualNow = dayjs().subtract(1, "hour");
			const shift = actualNow.diff(dayjs.unix(fixedNow), "seconds");
			return fixedBegin + shift;
		})(),
		DATASET_END_SECONDS: (() => {
			const fixedNow = timeConstants.fixedNow || global.FIXED_NOW;
			const actualNow = dayjs().subtract(1, "hour");
			const shift = actualNow.diff(dayjs.unix(fixedNow), "seconds");
			return fixedNow + shift;
		})(),
	};
