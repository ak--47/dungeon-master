 ...
 *   2. Neither provided → use (run_start - numDays, run_start), captured once in
 *      unix seconds. Pin both boundaries for deterministic reruns. Warn once.
...
function resolveDatasetWindow(datasetStart, datasetEnd, userNumDays, verbose = false) {
	...
	const runStartUnix = dayjs().unix();
	const fallbackStart = runStartUnix - fallbackNumDays * 86400;
	if (verbose) console.warn(
		`⚠️  No 'datasetStart'/'datasetEnd' set — dataset window anchored to this run's start time and will shift across runs. Pin both for full determinism.`
	);
	return { datasetStartUnix: fallbackStart, datasetEndUnix: runStartUnix, numDays: fallbackNumDays };
}
