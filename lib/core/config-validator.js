...
export function validateDungeonConfig(config) {
	...
	// ── Phase 2: Validate and resolve advanced features ──
...
function resolveWorldEvents(worldEvents, numDays) {
	...
	const beginUnix = global.FIXED_BEGIN;
	...
	return worldEvents.map(we => {
		...
		const resolved = { ...we, startUnix, endUnix };
		if (we.aftermath) {
			resolved.aftermathEndUnix = endUnix + (we.aftermath.duration * 86400);
		}
