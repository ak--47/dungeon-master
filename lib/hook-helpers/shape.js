 ...
 * <28min); inter-session gaps stay strictly over it (sessions land on distinct
 ...
 * Placement stays inside inclusive dataset bounds when supplied; omitted
 * bounds use the original stream's min/max timestamps, not inferred dataset
 * boundaries. Partial days compress clusters, including zero-span clusters.
 * Insufficient per-week capacity throws RangeError before any record changes;
 * the helper never silently reduces the session target or loses events.
 *
 * @param {Array<Object>} events - Full user event array (everything hook).
 ...
 * @throws {RangeError} Invalid bounds or insufficient 30-minute-session capacity.
...
export function applySessionShape(events, uid, opts) {
	...
	const lowerMs = datasetStart === undefined ? firstMs : toMs(datasetStart);
	const upperMs = datasetEnd === undefined ? lastMs : toMs(datasetEnd);
	...
	for (let s = 0; s < chunkSizes.length; s++) {
		...
		const span = Math.floor(Math.min(sesMs, (slotEnd - slotStart) * 0.5));
		const center = slotStart + Math.floor((slotEnd - slotStart - span) / 2);
		const q = Math.floor((slotEnd - slotStart - span) / 4);
