...
FROM read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true)
...
api_calls AS (
  ...
  FROM read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true) e
...
WITH per_user AS (
  SELECT user_id, MIN(time::TIMESTAMP) AS t0,
    COUNT(*) FILTER (WHERE event = 'rate limit error' AND time::TIMESTAMP < (SELECT MIN(time::TIMESTAMP) + INTERVAL '7 days' FROM read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true) e2 WHERE e2.user_id = e.user_id)) AS early_rl,
    COUNT(*) FILTER (WHERE time::TIMESTAMP <= (SELECT MIN(time::TIMESTAMP) + INTERVAL '7 days' FROM read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true) e2 WHERE e2.user_id = e.user_id)) AS pre,
    COUNT(*) FILTER (WHERE time::TIMESTAMP > (SELECT MIN(time::TIMESTAMP) + INTERVAL '7 days' FROM read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true) e2 WHERE e2.user_id = e.user_id)) AS post
  FROM read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true) e
  GROUP BY user_id
)
...
WITH per_user AS (
  SELECT user_id, MIN(time::TIMESTAMP) AS t0,
    BOOL_OR(event = 'eval job' AND time::TIMESTAMP < (SELECT MIN(time::TIMESTAMP) + INTERVAL '7 days' FROM read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true) e2 WHERE e2.user_id = e.user_id)) AS early_eval,
    COUNT(*) FILTER (WHERE time::TIMESTAMP > (SELECT MIN(time::TIMESTAMP) + INTERVAL '30 days' FROM read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true) e2 WHERE e2.user_id = e.user_id)) AS post30
  FROM read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true) e
  GROUP BY user_id
)
...
docs_between AS (
  ...
  FROM per_user p
  JOIN read_json_auto('data/verify-ai-EVENTS.json', sample_size=-1, union_by_name=true) e USING (user_id)
