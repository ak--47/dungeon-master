...
FROM read_json_auto('data/verify-insurance-application-EVENTS*.json', sample_size=-1, union_by_name=true)
...
users AS (SELECT distinct_id AS user_id, risk_profile FROM read_json_auto('data/verify-insurance-application-USERS*.json', sample_size=-1, union_by_name=true))
...
early_docs AS (
  ...
  FROM per_user p
  JOIN read_json_auto('data/verify-insurance-application-EVENTS*.json', sample_size=-1, union_by_name=true) e USING (user_id)
