#!/usr/bin/env node
/**
 * ai-platform.verify.mjs — thin wrapper around the story runner.
 *
 * All verification logic lives in the `stories` export of ./ai-platform.js;
 ...
 * Run:
 *   node dungeons/vertical/ai-platform/ai-platform.verify.mjs [prefix]   # default verify-ai-platform
 */
...
if (!events.length) {
	console.error(`no shards at ${prefixPath}-EVENTS*.json — run: node scripts/verify-runner.mjs dungeons/vertical/ai-platform/ai-platform.js ${PREFIX}`);
	process.exit(1);
}
console.log(`ai-platform — events=${events.length} users=${profiles.length} (${prefixPath})`);
