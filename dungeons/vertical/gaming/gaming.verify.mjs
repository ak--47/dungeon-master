#!/usr/bin/env node
/**
 * gaming.verify.mjs — thin wrapper around the story runner.
 *
 * All verification logic lives in the `stories` export of ./gaming.js;
 ...
 * Run:
 *   node dungeons/vertical/gaming/gaming.verify.mjs [prefix]   # default verify-gaming
 */
...
if (!events.length) {
	console.error(`no shards at ${prefixPath}-EVENTS*.json — run: node scripts/verify-runner.mjs dungeons/vertical/gaming/gaming.js ${PREFIX}`);
	process.exit(1);
}
console.log(`gaming — events=${events.length} users=${profiles.length} (${prefixPath})`);
