#!/usr/bin/env node
/**
 * devtools.verify.mjs — thin wrapper around the story runner.
 *
 * All verification logic lives in the `stories` export of
 * ./devtools.js; this script just streams the shards and
 * delegates. It is equivalent to:
 *
 *   node scripts/verify-stories.mjs dungeons/vertical/devtools/devtools.js --data-prefix verify-devtools
 *
 * Generate first:
 *   node scripts/verify-runner.mjs dungeons/vertical/devtools/devtools.js verify-devtools
 * Run:
 *   node dungeons/vertical/devtools/devtools.verify.mjs [prefix]   # default verify-devtools
 */
...
if (!events.length) {
	console.error(`no shards at ${prefixPath}-EVENTS*.json — run: node scripts/verify-runner.mjs dungeons/vertical/devtools/devtools.js ${PREFIX}`);
	process.exit(1);
}
console.log(`devtools — events=${events.length} users=${profiles.length} (${prefixPath})`);
...
// that cancel soup/persona confounds); H10 dispatches to emulateBreakdown's
// timeToConvert — funnel-post gap scaling is only visible to funnel-instance
