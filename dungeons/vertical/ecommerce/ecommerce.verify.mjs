#!/usr/bin/env node
/**
 * ecommerce.verify.mjs — thin wrapper around the story runner.
 *
 * All verification logic lives in the `stories` export of ./ecommerce.js;
 ...
 * Run:
 *   node dungeons/vertical/ecommerce/ecommerce.verify.mjs [prefix]   # default verify-ecommerce
 */
...
if (!events.length) {
	console.error(`no shards at ${prefixPath}-EVENTS*.json — run: node scripts/verify-runner.mjs dungeons/vertical/ecommerce/ecommerce.js ${PREFIX}`);
	process.exit(1);
}
console.log(`ecommerce — events=${events.length} users=${profiles.length} (${prefixPath})`);
