#!/usr/bin/env node
/**
 * food-delivery.verify.mjs — thin wrapper around the story runner.
 *
 * All verification logic lives in the `stories` export of
 * ./food-delivery.js; this script just streams the shards and
 * delegates. It is equivalent to:
 *
 *   node scripts/verify-stories.mjs dungeons/vertical/food-delivery/food-delivery.js --data-prefix verify-food-delivery
 *
 * Generate first:
 *   node scripts/verify-runner.mjs dungeons/vertical/food-delivery/food-delivery.js verify-food-delivery
 * Run:
 *   node dungeons/vertical/food-delivery/food-delivery.verify.mjs [prefix]   # default verify-food-delivery
 */
...
if (!events.length) {
	console.error(`no shards at ${prefixPath}-EVENTS*.json — run: node scripts/verify-runner.mjs dungeons/vertical/food-delivery/food-delivery.js ${PREFIX}`);
	process.exit(1);
}
console.log(`food-delivery — events=${events.length} users=${profiles.length} (${prefixPath})`);
...
// All stories are DuckDB-backed. Every read here is a ratio-of-ratios
// (delivered-per-placed, reorders-per-delivered, coupons-per-checkout) or a
// behavioral-cohort aggregation (hash bucket, hook-time order-count bucket,
// trial-activation split) — shapes emulateBreakdown's segmentation types
// don't model, and the RoR normalization is exactly what cancels the soup
